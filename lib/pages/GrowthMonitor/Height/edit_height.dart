...
class EditHeightPage extends StatefulWidget {
  @override
  _EditHeightPageState createState() => _EditHeightPageState();
}
...
class _EditHeightPageState extends State<EditHeightPage> {
  ...
  Future<void> saveMeasurement() async {
    if (selectedDate == null || heightController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Please select a date and enter height."),
    ...
    try {
      ...
      Map<String, dynamic> existingHeightData = {};

      if (userData.containsKey('height_data') &&
          userData['height_data'] is Map) {
        userData['height_data'].forEach((key, value) {
          existingHeightData[key.toString()] =
              value is num ? value.toDouble() : 0.0;
        });
      }
      String dateKey = selectedDate!.toIso8601String();
      double heightValue = double.tryParse(heightController.text) ?? 0.0;
      existingHeightData[dateKey] = heightValue;

      await firestore.collection('users').doc(userId).set({
        'height_data': existingHeightData,
        'latest_height_measurement': {
          'height': heightValue,
          'age': ageInYears,
          'date': dateKey,
        },
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: Icon(Icons.height, color: Colors.blue[900]),
                    ),
                  ),
                ),
                SizedBox(height: 30),
